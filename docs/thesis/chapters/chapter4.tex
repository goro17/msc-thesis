...
indispensable, and document the development of a Proof-of-Concept application built
...
\begin{itemize}
	\item \textit{Regular node}: is in charge of accepting and handling the user
		requests, employing both frontend and backend logic. It is implemented through
		...
		invokes a custom-made Python library which implements and executes the backend
...
into three core elements, which are the \textit{backend} engine, the \textit{frontend}
...
The backend serves as the main computational engine for each node in the network. As
discussed above, the backend logic is encapsulated through a custom-made library that
...
\textit{cryptography} library\footnote{https://github.com/pyca/cryptography}.
...
Upon retrieval of CRDT update containing the new file entry, if the node's backend
...
each time the class is instantiated -- generally, this only occurs when the backend
...
CRDTSign's \texttt{api.py} module serves as the user's entry point to the application. Its
...
\begin{figure}[tb!]
    ...
    \caption{The main view for CRDTSign's frontend.}
...
file's signature. This request is dispatched to the backend using the available
...
containers used to deploy software components individually. More specifically, the Docker
image that is installed in each deployed container is created using the following script
-- or \textit{Dockerfile}.
...
Once the image corresponding to the above Dockerfile is created, it is used to deploy a
set of containers that utilize CRDTSign. Moreover, the solution was tested by invoking
the Docker Compose commmand to deploy the cluster of containers specified by the
following configuration file -- named \texttt{docker-compose.yml} -- which deploys
a simple cluster made up of four containers; one for the relay server, and the others
for the "user" nodes.
...
As an addendum to the provided solution, a prototype mobile frontend application was
developed to wrap around the present backend logic. The application was written using
...
present in the main frontend application described in \autoref{sec:frontend}. Namely,
