...
\begin{itemize}
	\item User identity management (module \textit{user.py})
	\item Cryptographic signature generation and validation (module \textit{sign.py}).
	\item CRDT state management and persistence (module \textit{storage.py}).
	\item Additional utility operations, such as file serialization and deserialization
		(module \textit{file_utils.py}), and operations concerning the enforcement of
		ad-hoc data retention policies (module \textit{data_retention.py}).
\end{itemize}

\subsubsection{user.py -- User identity management}

The \textit{user.py} module curates the identity management and persistence on local
...
\begin{itemize}
	\item the user's \textit{unique identifier} (\textit{user_id}), which is generated by
	...
	\item the user's \textit{displayed name} (\textit{username}), which is inputted by
...
placed in a location which is accessible to the \textit{storage.py} module -- and as we
...
\subsubsection{sign.py -- Cryptographic signature generation and validation}

The \textit{sign.py} module provides a collection of Python methods that enable the
...
\textit{cryptography} library\footonote{\url{https://github.com/pyca/cryptography}}.
...
\textit{storage.py} module for attachment to the target file's metadata.

The module also provides a custom signature \textit{validation} method, which
requires three inputs:
\begin{itemize}
	\item the target file's hash digest;
	\item the target file's signature;
	\item the public key associated to the target file's signing user.
\end{itemize}
...
outcome of the verification -- $True$ if the signature is deemed \textit{valid} with the
provided parameters, $False$ otherwise.

\subsubsection{storage.py -- CRDT state management and persistence}
...
within a dedicated abstract class: the \textit{FileSignatureStorage} class and the
\textit{UserStorage} class. These classes implement state management performed both
...
\paragraph{FileSignatureStorage}
The \textit{FileSignatureStorage} class is employed to manage the files shared among
nodes in dedicated CRDTs modeled as key-value stores -- implemented using the
\textit{pycrdt} library's \textit{Map} object. The key for each item in the CRDT is
...
An additional method is provided within the \textit{FileSignatureStorage} class, which
...
\begin{itemize}
	...
	\item if $t_{now} \lt t_{retention}$, this means that the file's retention has not
		yet expired. No action is taken by the system in this case.
\end{itemize}

\paragraph{UserStorage}

Similar to what occurs for the \textit{FileSignatureStorage} class, a dedicated CRDT
key-value store is reserved within the \textit{UserStorage} class, to collect information
...
\subsubsection{file_utils.py -- File blob serialization and deserialization}

File blobs that are placed in the CRDT key-value store employed by the
\textit{FileSignatureStorage} class are subject to custom serialization and
deserialization procedures. For instance, the \textit{serialization} operation takes the
target file's binary data as input, and applies the \textit{LZ4} compression algorithm
\footnote{\url{https://lz4.org/}} to the retrieved data. The output of the compression
algorithm is the payload that is embedded as the target file's blob in the metadata saved
inside the CRDT. Conversely, the \textit{deserialization} operation takes the file's blob
as input, and decompresses it via the same LZ4 algorithm, returning the original file's
content as output.
