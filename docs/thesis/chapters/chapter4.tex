...
supports all core functionalities of the solution -- namely, the logical layers
concerning CRDT state management operations, data persistence, and communication with
the relay server.

\subsubsection{CRDT State Management}

State management is handled by invoking the pycrdt library to generate dedicated,

\begin{itemize}
	\item The back-end connects to the relay server, as to notify 
	\item The back-end creates a new file in local storage, which serves as a container
		for relevant information concerning the user's identity, such as its
		\textit{name} and its \textit{unique identifier} -- this action is performed only
		once for each new user. While the user is prompted to insert its assigned name
		through the front-end, the unique identifier is automatically assigned by the
		back-end, which generates a version 4 Universally Unique IDentifier
		\cite{uuidv4_rfc} (\textit{UUIDv4}). If the user's information was already saved
		on local storage, the back-end loads the user's details from that file, rather
		than creating a new one. Furthermore, the back-end generates a
		\textit{cryptographic keypair} for such user, which is made up of a
		\textit{private key} and a \textit{public key}. The latter key will prove to be
		fundamental for implmenting the signature validation procedure; as such, the
		back-end shares the public key with the other nodes by placing it in a dedicated
		CRDT, modeled as a key-value store -- where the key is the user's unique ID, and
		the value is the user's public key itself. The selected scheme for generating the
		cryptographic keypair is illustrated in \autoref{sec:dss}.
\end{itemize}
