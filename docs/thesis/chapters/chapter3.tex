...
In implementing YATA, Yjs provides a \textit{Document} structure, which serves as the
core of the CRDT implementation. The Yjs Document is essentially a container of shared
data types -- maps, arrays, text, ... -- that are locally updated by triggering a
so-called \textit{transaction}. The updates generated by one or more transactions are
entities that can be propagated over the chosen broadcast protocol, and be seamlessly
merged to the state of other devices. In compliance with the decentralized nature of
CRDTs, a replica can autonomously apply a received update to its internal state, and does
not require coordination with any external entity -- e.g., a centralized coordination
server.
