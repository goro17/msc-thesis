...
$s_2$ resulting from concurrent write operations, the CRDT's \textit{merge} operation is
applied as follows: $$\text{merge}(s_1,s_2) = \text{max}(s_1\text{.timestamp},
s_2\text{.timestamp})$$

The merge operation compares the timestamp from concurrent update and retains the
state with the highest timestamp. This forms a semilattice where the partial
order is defined by timestamp comparison, ensuring deterministic convergence regardless
of message delivery order. An example of state-based LWW-Register
\cite{shapiro2011comprehensive} can be found in specification 0.

\begin{algorithm}
    \caption{State-based LWW-Register}
    \Input{Payload}
    \Output{None}
...
\begin{itemize}
    \item $\text{increment}()$ updates $P[\text{local.id}]$
    \item $\text{decrement}()$ updates $N[\text{local.id}]$
\end{itemize}
