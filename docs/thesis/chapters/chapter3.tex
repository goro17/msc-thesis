...
The \textit{Replicated Growable Array} implements sequences in the form of a linked list.
Each node of the list holds an element and an identifier associated to it, plus a
reference to its parent node \cite{Roh2011RGA} -- i.e., the node after which it was
inserted.
...
This definition of precedence is crucial understanding concurrency semantics for
...
As a practical example of this behavior, consider a system formed by two replicas, $A$
and $B$, each maintaining a copy of an RGA. The initial state of both replicas is shown
in [...] % TODO: continue example
