...
1. A user connects to a node in the network via the web browser of their preference.
    - If the user is accessing the application for the first time, the system creates a new user profile, assigning it a unique identifier and key pair (private and public keys, generated using the Ed25519 scheme) and saves the information locally.
...
2. The system displays the user interface in the web browser. Within the interface, the user can perform the following actions:
    ...
    > accordingly.
    - **Manually Validate a Signature**: The user can manually input the relevant information required to validate a file signature; namely, the file itself (of which the system computes the SHA-256 digest), the file's signature, and the signing user's public key. The system applies the Ed25519 signature verification algorithm over the file's signature, and displays a positive outcome if the verification algorithm is successful, or a negative outcome otherwise.